...
<html>
    ...
    <body>
        <div id="menu" class="demo1">
			...
			<!--画像送信スクリプト-->
			<script>
				function sendImage() """),format.raw/*126.26*/("""{"""),format.raw/*126.27*/("""
					$("#submit").trigger('click');
					$("#reset").trigger('click');
				"""),format.raw/*129.5*/("""}"""),format.raw/*129.6*/("""
				<!--背景画像選択ウィンドウの表示-->
				function toSelectBg() """),format.raw/*131.27*/("""{"""),format.raw/*131.28*/("""
			...
			<script>
			//var target = "null";
			function delConfirm()"""),format.raw/*139.25*/("""{"""),format.raw/*139.26*/("""
				var res = confirm("このページを削除しますか？");
				if(res) """),format.raw/*141.13*/("""{"""),format.raw/*141.14*/("""
				console.log(target);
				parent.main.location.href="edit_call/delete_page/"+target;
				"""),format.raw/*144.5*/("""}"""),format.raw/*144.6*/(""" else """),format.raw/*144.12*/("""{"""),format.raw/*144.13*/("""
